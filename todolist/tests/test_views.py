...
class ProjectListCreateApiViewTest(APITestCase):
    ...
    def test_should_throw_an_error_when_project_name_has_invalid_length(self):
        sample = self.data
        sample['name'] = "Test"
        response = self.client.post(path=self.url, data=self.data, format='json')
        data = response.json()
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEquals(data['data']['details']['error_code'], 'INPUT_VALIDATION_FAILURE')
    
    def test_should_throw_an_error_when_project_description_has_invalid_length(self):
        sample = self.data
    ...
    def test_get_projects_when_there_are_no_project_records(self):
